...
class Vector {
  ...
  isValid() {
    return (
      ...
      Math.abs(this.point1.x - this.point2.x) ===
  ...
  collectLinePoints() {
    ...
    if (dx === 0) {
      const yOrigin = Math.min(this.point1.y, this.point2.y)
      for (let y = 0; y <= dy; y++) {
        points.push(new Point(this.point1.x, yOrigin + y).toString())
      }
    } else if (dy === 0) {
      const xOrigin = Math.min(this.point1.x, this.point2.x)
      for (let x = 0; x <= dx; x++) {
      ...
      let x = this.point1.x
      let y = this.point1.y
      let xInc = this.point1.x < this.point2.x ? 1 : -1
      let yInc = this.point1.y < this.point2.y ? 1 : -1
      for (let counter = 0; counter <= dx; counter++) {
        points.push(new Point(x, y).toString())
        x += xInc
        y += yInc
      }
    }
